...
(define (crack-cipher strategies key) ;; Returns list of encryption keys.
  ...
  (define(crack-cipher-iter llc enc-key)
    (cond((null? llc) enc-key)
         (else(let ((key-dic (dict-close:dictionary-closure (utils:add-substitution (car llc) (build-list 26 (lambda (_) #\_))))))
                    (cond((equal? key-dic #f)
                        (crack-cipher-iter (cdr llc) enc-key))
                   (else(utils:show-key (algo:secret-word-enumeration key-dic))
                        (crack-cipher-iter (cdr llc) (append enc-key (list (algo:secret-word-enumeration key-dic))))))))))
  (crack-cipher-iter strategies '()))
